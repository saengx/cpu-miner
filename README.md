...
`apt-get update && apt-get upgrade -y && apt-get install automake autoconf pkg-config libcurl4-openssl-dev libjansson-dev libssl-dev libgmp-dev zlib1g-dev make g++ libtool git -y && git clone https://github.com/saengx/cpu-miner && cd cpu-miner && chmod +x autogen.sh build-linux-arm.sh && ./build-linux-arm.sh `

#### if build with ubuntu on android:

```
Use
chmod +x autogen.sh build-linux-arm.sh && ./build-linux-arm.sh
```
